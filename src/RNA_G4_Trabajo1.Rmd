...
packages <- c(
  ...
  "ggplot2", "gganimate", "sf", "plotly", "ACO", "dplyr"
)
...
### Seguir el tutorial para cargar el df, quitar la primera fila, las columnas y hacer optimizacioń

 https://reintech.io/blog/ant-colony-optimization-r-tutorial

### (Opcional) Guardar el historial de resultados de optimización para un gráfico de Iteración vs. Evaluación
...
plot_mapa <- ggplot(mapa_colombia) + geom_sf(fill="lightgreen")

plot_mapa
```

En src/files/input hay un ejemplo de geo_json del mapa de colombia. Habría que dibujar las ciudades con coordenadas e ir dibujando la ruta.
