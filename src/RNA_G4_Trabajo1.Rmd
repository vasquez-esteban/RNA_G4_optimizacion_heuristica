...
- [Introducción](#item-1)
- [Parte 1: Optimización Numérica](#item-2)
  - [Exploración de las funciones](#item-2-1)
    - [Función de Schwefel](#item-2-1-1)
    - [Función de Griewank](#item-2-1-2)
  - [Optimización por Descenso del Gradiente](#item-2-2)
  - [Optimización por Mëtodos Heurísticos](#item-2-3)
    - [Algoritmo Genético](#item-2-3-1)
    - [Optimización por Enjambre](#item-2-3-2)
    - [Evolución Diferencial](#item-2-3-3)
  - [Visualización de Resultados](#item-2-4)
    - [Animación Descenso por Gradiente](#item-2-4-1)
    - [Animación Métodos Heurísticos](#item-2-4-2)
  - [Conclusiones](#item-2-5)
    - [Métodos Clásicos](#item-2-5-1)
    - [Métodos Heurísticos](#item-2-5-2) 
      - [Algoritmos Genéticos](#item-2-5-2-1)
      - [Enjambres de partículas](#item-2-5-2-2)
      - [Evolución Diferencial](#item-2-5-2-3)
    - [Conclusión General](#item-2-5-3) 
- [Parte 2: Optimización Combinatoria](#item-3)
  - [Procedimiento de levantamiento de datos](#item-3-1)
    - [Definición de ciudades principales](#item-3-1-1)
    - [Construcción de Matriz de Costos](#item-3-1-2)
    - [Vehículos seleccionado](#item-3-1-3)
    - [Cálculo del costo de combustible](#item-3-1-4)
    - [Cáclulo del costo de la hora del vendedor](#item-3-1-5)
    - [Obtención de tiempos y distancias](#item-3-1-6)
    - [Obtención de costos de peajes](#item-3-1-7)
  - [Optimización de la obtención de datos](#item-3-2)
  - [Gráfico de la ruta óptima](#item-3-3)
  - [Conclusiones](#item-3-4)
- [Reporte de Contribución Individual](#item-4)
- [Repositorio en GitHub](#item-5)
- [Bibliografía](#item-6)
...
```{r carga_paquetes, echo=FALSE}
packages <- c(
  "numDeriv", "GA", "pso", "DEoptim",
  "ggplot2", "gganimate", "sf", "plotly", "dplyr", "animation", "tidyverse"
)
...
# Parte 1: Optimización Numérica <a id="item-2"></a>
...
## 0. Exploración de las funciones <a id="item-2-1"></a>


### Función de Schwefel <a id="item-2-1-1"></a>
...
```{r fig.cap="**Fig 2.** *Evaluación de Schwefel (3D)*", fig.align="center", out.width="70%", layout.align="center", echo=FALSE}
...
```{r fig.cap="**Fig 3.** _Curvas de Nivel de Schwefel_", fig.align="center", out.width="70%", echo=FALSE, warning=FALSE}
...
### Función de Griewank <a id="item-2-1-2"></a>
...
```{r fig.cap="**Fig 5.** *Evaluación de Griewank (3D)*", fig.align="center", out.width="70%", echo=FALSE}
...
```{r fig.cap="**Fig 6.** _Curvas de Nivel de Griewank_", fig.align="center", out.width="70%", echo=FALSE, warning=FALSE}
# Crear una grilla de puntos (para 2D)
...
## 1. Optimización por descenso del gradiente o métodos Quasi-Newton <a id="item-2-2"></a>
...
### 2. Métodos Heurísticos <a id="item-2-3"></a>
...
#### 2.1 Algoritmo Genético (GA) <a id="item-2-3-1"></a>

```{r Algoritmo Genético, echo=FALSE}
...
#### 2.2 Optimización por Enjambre (PSO) <a id="item-2-3-2"></a>

```{r Optimización por Enjambre, echo=FALSE}
...
#### 2.3 Evolución Diferencial (DE) <a id="item-2-3-3"></a>

```{r Evolución Diferencial, echo=FALSE}
...
## 3. Visualización de Resultados <a id="item-2-4"></a>

### 3.1 Animación del Descenso de Gradiente <a id="item-2-4-1"></a>
...
```{r fig.cap="**Fig 7.** *Camino de la solución en Schwefel 2D*", fig.align="center", out.width="70%", echo=FALSE}
...
p <- ggplot() +
  ...
  annotate("text",
    x = 420.9687, y = 450, label = "Óptimo Global",
...
```{r fig.cap="**Fig 8.** *Camino de la solución en Schwefel 3D*", fig.align="center", out.width="70%", echo=FALSE}
...
```{r fig.cap="**Fig 9.** *Camino de la solución en Griewank 2D*", fig.align="center", out.width="70%", echo=FALSE}
...
p <- ggplot() +
  ...
  # Trayectoria del optimizador
  geom_path(
    data = griewank_df_3d, aes(x = x, y = y),
    color = "darkgrey", alpha = 0.5, linewidth = 0.8
  ) +

  # Puntos animados (nuevo esquema de colores)
  geom_point(
    data = griewank_df_3d, aes(x = x, y = y, color = step),
    size = 3, show.legend = FALSE
  ) +
  scale_color_gradient(low = "orange", high = "red") + # Azul a verde
...
```{r fig.cap="**Fig 10.** *Camino de la solución en Griewank 3D*", fig.align="center", out.width="70%", echo=FALSE}
knitr::include_graphics("files/griewank_3d_path.gif")
```

### 3.2 Animación de Métodos Heurísticos <a id="item-2-4-2"></a>

```{r animación métodos heurísticos, echo=FALSE}
...
plot_trajectory_animation <- function(history_df, grid_df, func, title, output_file) {
  ...
  if (!any(is.na(opt_point))) {
    p <- p +
      ...
      annotate("text",
        x = opt_point[1], y = optimal_label_y, label = "Óptimo Global",
        color = "cyan", fontface = "bold"
      )
  }
...
```{r fig.cap="**Fig 11.** *GA Schwefel 2D*", fig.align="center", out.width="70%", echo=FALSE}
...
```{r fig.cap="**Fig 12.** *GA Schwefel 3D*", fig.align="center", out.width="70%", echo=FALSE}
...
```{r fig.cap="**Fig 13.** *GA Griewank 2D*", fig.align="center", out.width="70%", echo=FALSE}
...
```{r fig.cap="**Fig 14.** *GA Griewank 3D*", fig.align="center", out.width="70%", echo=FALSE}
...
```{r fig.cap="**Fig 15.** *PSO Schwefel 2D*", fig.align="center", out.width="70%", echo=FALSE}
...
```{r fig.cap="**Fig 16.** *PSO Schwefel 3D*", fig.align="center", out.width="70%", echo=FALSE}
...
```{r fig.cap="**Fig 17.** *PSO Griewank 2D*", fig.align="center", out.width="70%", echo=FALSE}
...
```{r fig.cap="**Fig 18.** *PSO Griewank 3D*", fig.align="center", out.width="70%", echo=FALSE}
...
```{r fig.cap="**Fig 19.** *DE Schwefel 2D*", fig.align="center", out.width="70%", echo=FALSE}
...
```{r fig.cap="**Fig 20.** *DE Schwefel 3D*", fig.align="center", out.width="70%", echo=FALSE}
...
```{r fig.cap="**Fig 21.** *DE Griewank 2D*", fig.align="center", out.width="70%", echo=FALSE}
...
```{r fig.cap="**Fig 22.** *DE Griewank 3D*", fig.align="center", out.width="70%", echo=FALSE}
knitr::include_graphics("files/de_griewank_3d.gif")
```

## 4. Conclusiones <a id="item-2-5"></a>

### 4.1. Métodos Clásicos (Descenso del Gradiente) <a id="item-2-5-1"></a>
...
### 4.2. Métodos Heurísticos <a id="item-2-5-2"></a>
...
#### 4.2.1. Algoritmos Genéticos <a id="item-2-5-2-1"></a>
...
#### 4.2.2. Optimización por Enjambre de Partículas (PSO) <a id="item-2-5-2-2"></a>
...
#### 4.2.3. Evolución Diferencial <a id="item-2-5-2-3"></a>
...
### 4.3. Conclusión General <a id="item-2-5-3"></a>
...
# Parte 2: Optimización Combinatoria <a id="item-3"></a>
...
### Procedimiento de levantamiento de datos <a id="item-3-1"></a>

#### Definición de ciudades principales <a id="item-3-1-1"></a>
...
### Construcción de la matriz de costos <a id="item-3-1-2"></a>
...
### Vehículo seleccionado <a id="item-3-1-3"></a>
...
### Cálculo del costo del combustible <a id="item-3-1-4"></a>
...
### Cálculo del costo de la hora del vendedor <a id="item-3-1-5"></a>
...
### Obtención de tiempos y distancias <a id="item-3-1-6"></a>
...
### Obtención de costos de peajes <a id="item-3-1-7"></a>
...
### Optimización de la obtención de datos <a id="item-3-2"></a>
...
costos <- as.matrix(read.csv("files/input/matriz_costos.csv",
  header = TRUE,
  sep = ",",
  stringsAsFactors = FALSE
))
...
ciudades_coord <- data.frame(
  "nombre" = c(
    "Palmira", "Pasto", "Tuluá", "Bogotá", "Pereira", "Armenia", "Manizales",
    "Valledupar", "Montería", "Soledad", "Cartagena", "Barranquilla",
    "Medellín", "Bucaramanga", "Cúcuta"
  ),
  "long" = c(
    -76.5, -77.3, -76.2, -74.1, -75.7, -75.7, -75.5, -73.3, -75.9, -74.8,
    -75.5, -74.8, -75.6, -73.1, -72.5
  ),
...
optimizacion_colonia_hormigas <- function(costos,
                                          num_hormigas = 50,
                                          num_iteraciones = 100,
                                          alpha = 1,
                                          beta = 5,
  ...
  for (iter in 1:num_iteraciones) {
    ...
    for (hormiga in 1:num_hormigas) {
      for (i in 1:(length(conjunto_rutas[[hormiga]]) - 1)) {
        feromonas[conjunto_rutas[[hormiga]][i], conjunto_rutas[[hormiga]][i + 1]] <-
          feromonas[conjunto_rutas[[hormiga]][i], conjunto_rutas[[hormiga]][i + 1]] + Q / conjunto_distancias[hormiga]
      }
...
### Crear un gráfico que dibuje la ruta óptima.  <a id="item-3-3"></a>

```{r echo=FALSE}
df_solucion_vecino <-
  data.frame(ciudades_coord$nombre[vmc_ruta], ciudades_coord$long[vmc_ruta], ciudades_coord$lat[vmc_ruta])
df_solucion_colonia <-
  data.frame(ciudades_coord$nombre[aco_ruta], ciudades_coord$long[aco_ruta], ciudades_coord$lat[aco_ruta])
      ...
      for (i in 1:nrow(df_solucion_vecino)) {
        ...
        plot(
          ggplot(st_geometry(mapa_colombia)) +
            ...
            geom_label(
              data = ciudades_coord, aes(x = long, y = lat, label = nombre),
              size = 3, label.size = NA, fill = NA
            ) +
            ylim(0, 11.5) +
            xlim(-78, -72) +
            geom_path(
              data = ciudades_recorridas, aes(
                x = ciudades_coord.long.vmc_ruta.,
                y = ciudades_coord.lat.vmc_ruta.
              ),
              colour = "red"
            )
        )
      ...
      for (i in 1:nrow(df_solucion_colonia)) {
        ...
        plot(
          ggplot(st_geometry(mapa_colombia)) +
            ...
            geom_path(
              data = ciudades_recorridas, aes(
                x = ciudades_coord.long.aco_ruta.,
                y = ciudades_coord.lat.aco_ruta.
              ),
              colour = "red"
            )
        )
...
guardar_gif_colonia()
```
```{r fig.cap="**Fig 22.** *Mejor Ruta usando Colonia de Hormigas*", fig.align="center", out.width="70%", echo=FALSE}
knitr::include_graphics("ruta_colonia_hormigas.gif")
```

```{r fig.cap="**Fig 23.** *Mejor Ruta usando K-Vecino más cercano*", fig.align="center", out.width="70%", echo=FALSE}
knitr::include_graphics("ruta_vecino_más_cercano.gif")
```

### Conclusiones <a id="item-3-4"></a>
...
## Reporte de Contribución Individual <a id="item-4"></a>
...
## Repositorio en Github <a id="item-5"></a>

-   [Click Derecho Y Abrir en Nueva Pestaña](https://github.com/vasquez-esteban/RNA_G4_optimizacion_heuristica)
 
## Bibliografía <a id="item-6"></a>
...
- Scrucca, L. (2013). GA: A Package for Genetic Algorithms in R. Journal of Statistical Software, 53(4), 1–37. https://cran.r-project.org/package=GA
  ↳ Utilizado para implementar el algoritmo genético (GA) y guardar el historial de soluciones en cada iteración.

- Bendtsen, C. (2012). DEoptim: An R Package for Global Optimization by Differential Evolution. Retrieved April 2025, from https://cran.r-project.org/package=DEoptim
  ↳ Usado para aplicar el método de evolución diferencial y extraer la mejor solución por iteración.

- Bendtsen, C. (2012). pso: Particle Swarm Optimization. Retrieved April 2025, from https://cran.r-project.org/package=pso
  ↳ Herramienta utilizada para implementar la optimización por enjambre de partículas (PSO) en 2 y 3 dimensiones.

- Surjanovic, S., & Bingham, D. (n.d.). Schwefel Function (schwef). Simon Fraser University. Retrieved April 2025, from https://www.sfu.ca/~ssurjano/schwef.html
  ↳ Fuente utilizada para la definición matemática, dominio y mínimo global de la función de Schwefel.

- Surjanovic, S., & Bingham, D. (n.d.). Griewank Function (griewank). Simon Fraser University. Retrieved April 2025, from https://www.sfu.ca/~ssurjano/griewank.html
  ↳ Fuente utilizada para la descripción formal, propiedades y representación de la función de Griewank.
...
- Dorigo, M., & Blum, C. (2005). Ant colony optimization theory: A survey. Theoretical Computer Science, 344(2–3), 243–278. https://doi.org/10.1016/j.tcs.2005.05.020
  ↳ Fuente académica que describe el algoritmo de optimización por colonia de hormigas (ACO), utilizado como base para la implementación en este trabajo.
