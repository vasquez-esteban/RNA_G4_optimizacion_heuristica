...
plot_mapa <- ggplot(st_geometry(mapa_colombia)) + 
  geom_sf(fill="lightgreen") +
  geom_point(data = ciudades_coord, aes(x = long, y = lat), colour = "red")+
  geom_label(data = ciudades_coord, aes(x = long, y = lat, label=nombre), vjust = -0.5, size = 3,label.size = NA,fill = NA)+
  ylim(0,11.5)+xlim(-78,-68)
